...
def draw_license_plate_rectangles(image, license_plate_contours, color=(0, 255, 0), thickness=2):
    """
    绘制车牌位置的矩形框

    :param image: 原始图像
    :param license_plate_contours: 车牌位置信息列表
    :param color: 矩形框的颜色，默认为绿色
    :param thickness: 矩形框的线条粗细，默认为2
    :return: 绘制了矩形框的图像
    """
    image_with_rectangles = image.copy()

    for contour_info in license_plate_contours:
        # 从轮廓信息中提取最小外接矩形
        center, size, angle = contour_info
        ...
        # 绘制矩形框
        cv2.drawContours(image_with_rectangles, [box], 0, color, thickness)
...
def img_findContours(img_contours):
    ...
    print("car_contours{}".format(car_contours))
    return car_contours
