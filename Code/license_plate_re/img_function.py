...
import cv2
import numpy as np
from Code.license_plate_re import img_math as img_math  # 本来是import img_math 即可，但是在flask中引用需要修改一下，不然报错说没有该模块
...
class CardPredictor:
    ...
    @staticmethod
    def img_first_pre(car_pic_file):
        """
        ...
        cv2.imshow("img_edge.jpg", img_edge)
        cv2.waitKey(0)
        ...
        cv2.imshow("img_xingtai.jpg", img_edge2)
        cv2.waitKey(0)
    ...
    def img_only_color(self, filename, oldimg, img_contours):
        ...
        # 绘制矩形框并获取结果图像
        # 框出车牌位置
        image_with_rectangles = img_math.draw_license_plate_rectangles(filename, card_contours)
