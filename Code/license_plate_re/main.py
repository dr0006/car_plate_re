...
class UI_main(ttk.Frame):
    ...
    def get_imgtk(self, img_bgr):
        ...
        pil_image_resized = im.resize((500, 400), Image.BOX)
    ...
    def from_pic(self):
        ...
        self.clean()
        if self.pic_path == '':
            messagebox.showwarning("提示", "您未选择任何车牌图片")
        self.pic_source = "本地文件：" + self.pic_path
        self.pic(self.pic_path)
        print(self.colorimg)

    def show_roi(self, r, roi, color):  # 车牌定位后的图片
        print("Entering show_roi")
        if r:
            try:
                print("Converting roi to RGB")
                roi = cv2.cvtColor(roi, cv2.COLOR_BGR2RGB)
                roi = Image.fromarray(roi)
                print("Resizing roi image")
                pil_image_resized = roi.resize((200, 50), Image.BOX)
                print("Creating Tkinter PhotoImage")
                self.tkImage2 = ImageTk.PhotoImage(image=pil_image_resized)
                print("Configuring roi_ct2")
    ...
    def clean(self):
        ...
        pil_image_resized = self.pilImage3.resize((200, 50), Image.BOX)
